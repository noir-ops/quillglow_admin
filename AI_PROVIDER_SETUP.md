...
# Optional third provider — any endpoint that speaks the OpenAI
# chat-completions format (self-hosted Ollama/vLLM/LM Studio, or a hosted
# open-weight provider like Together, Groq, OpenRouter).
OPEN_SOURCE_MODEL_API_KEY=...   # optional — many self-hosted servers need none
```
...
Run `scripts/006_add_opensource_ai_provider.sql` (same file ships with the
QuillGlow app as `056_...`) to add the `opensource_base_url` /
`opensource_model` columns before this option appears usable.

## 3. Using the page

- **Active provider** — radio buttons for ChatGPT (OpenAI), Gemini (Google),
  or an open-source model. Only one is live at a time; the others keep their
  saved settings so you can flip between them instantly.
...
- **Open-source model** — enter the endpoint's base URL (e.g. an Ollama
  server's `http://localhost:11434/v1`, or a hosted provider's API base) and
  click "Fetch models" to try listing them via `/models`. Many self-hosted
  servers don't implement that endpoint — if the fetch fails, just type the
  model id your server expects into the text field that appears instead.
  There's no pricing catalog for this option since it varies per deployment.
...
- **Test connection** — sends one tiny prompt and reports the reply and latency,
  so you can verify a key/endpoint and model id before saving.
