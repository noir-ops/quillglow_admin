...
export default async function SubscriptionsPage({
  ...
  // Fetch one page of subscriptions plus an exact count.
  ...
  if (params.search) {
    subscriptionsQuery = subscriptionsQuery.or(
      `plan_type.ilike.%${params.search}%,polar_customer_id.ilike.%${params.search}%`,
    )
  }
  ...
  const { data: subscriptionsData, error: subscriptionsError, count } = await subscriptionsQuery

  if (subscriptionsError) {
    console.error("Error fetching subscriptions:", subscriptionsError)
  }

  // Only fetch the profiles referenced by THIS page's rows. Previously this
  // pulled every profile on the platform on every render just to build a
  // lookup map for ~50 visible rows.
  const userIds = [...new Set((subscriptionsData ?? []).map((s) => s.user_id).filter(Boolean))]
  ...
  // Status counts via head-only COUNT queries rather than filtering a
  // full in-memory copy of the table.
  const statusCount = (status: string) =>
    supabase.from("subscriptions").select("id", { count: "exact", head: true }).eq("status", status)

  const [activeRes, canceledRes, trialingRes, cancelingRes] = await Promise.all([
    statusCount("active"),
    statusCount("canceled"),
    statusCount("trialing"),
    statusCount("canceling"),
  ])

  const activeCount = activeRes.count ?? 0
  const canceledCount = canceledRes.count ?? 0
  const trialingCount = trialingRes.count ?? 0
  const scheduledCancelCount = cancelingRes.count ?? 0
